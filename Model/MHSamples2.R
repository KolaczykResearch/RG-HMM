...
get_proposalPath2 = function(netSeq_path, binSeq_path, N){
  ...
  # logQ_diff = log(choose(N, 2)) + log(choose(R-2, 2)) - log()
  # R = length - 1
...
getSamplePath = function(D, y1, w1, y2, w2, t1, t2, p, q, gamma, process = "ER", prop_max = 100){
  ...
  if (process == "PR"){
    ...
    # cat("Total_proposed_init:", prop_num_init, ".")
  }
  while (path_num <= D){
    temp = get_proposalPath2(netSeq_path_cur, binSeq_path_cur, N)
    prop_num = prop_num + 1
...
# For parallel over both M
# getSamplePath_parallel = function(processID, D, Y_true, W_true, t_obs, p_cur, q_cur,
#                                   gamma_cur, process = "ER", prop_max, burnIn = 0, burnIn_mcmc = 0){
#   source("Model/MHSamples.R", chdir=T)
#   M = length(t_obs)
#   sum_01 = 0
#   sum_0 = 0
#   sum_10 = 0
...
# For parallel over both H and M 
