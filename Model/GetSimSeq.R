...
get_SimSeq = function(p, q, gamma, N, t_obs, net_start=NULL, w_start=NULL, process = "ER"){
  ...
  network_seq_obs[[1]] = reformat(network_seq_obs[[1]], N)
...
network_change2 = function(network_cur, w_nex, N, process="ER", edgeIndex){
  ...
  if (process == "ER"){
    if (w_nex == 1){
      ...
      pos = sample_modified(network_cur, 1)
      network_nex = setdiff(network_cur, pos)
    }
  }else if (process == "PR"){
    ...
    network_nex = reformat(network_nex, N)
  }
  return(network_nex)
}

get_w_nex2 = function(w_cur, network_cur, p, q){
  ###--Get w ~ Bernoulli (p)/Bernoulli (1-q)
  if (length(network_cur) == 0){
    #if graph is empty, p = 1
    w = 1
  }else if (length(network_cur) == (N^2-N)/2){
    #if graph is full, q = 1
    w = 0
  }else{
    if (w_cur==1){
...
get_SimSeq2 = function(p, q, gamma, N, t_obs, net_start=NULL, w_start=NULL, process = "ER", edgeIndex){
  M = length(t_obs)
  network_seq_obs = list()
  bin_var_obs = NULL
  # start from an empty network if no initial network provided
  if (length(net_start) == 0){
    network_cur = numeric(0)
  }else{
    network_cur = net_start
  }
  # start from w_cur=1 if no initial provided
  ...
  while (m<M){
    ###--Get w_nex ~ Bernoulli (p)/Bernoulli (1-q)
    w_nex = get_w_nex2(w_cur, network_cur, p, q)
    ...
    ###--Add edge if w_nex=1, delete edge if w_nex=0
    network_nex = network_change2(network_cur, w_nex, N, process=process, edgeIndex)
