...
test_mcmc = function(net_obs, bin_obs, t_obs, process = "ER", D = 50, prop_max = 100,
  ...
  if(MHParallel){
    ...
    list_sum = matrix(0, M-1-burnIn, 5)
    for (h in 1:H){
      cat(h, ":", ids[h], "...,")
      b = ids[h]
  ...
  q_nex = pqgamma[2]
  gamma_nex = pqgamma[3]
  return(list(p_nex = p_nex, q_nex = q_nex, gamma_nex = gamma_nex, list_sum = list_sum))
}
